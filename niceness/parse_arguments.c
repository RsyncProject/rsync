 ...
 * parse_arguments.c - parse command line argument
 *
	...
	{   /* Location "local" or "remote" or "all" found */
		...
		{   /* Location followed by specific setting */
			...
			{   /* End of string or comma */
				...
				if (isLocal) {
					*nice_local = NICENESS_RENICE_DEFAULT_PRIO;
					*ionice_local = NICENESS_IONICE_DEFAULT_PRIO;
				}
				if (isRemote) {
					*nice_remote = NICENESS_RENICE_DEFAULT_PRIO;
					*ionice_remote = NICENESS_IONICE_DEFAULT_PRIO;
				}
				return pointer;
			} else
...
int parse_argument(const char *config_str, int *nice_local, int *ionice_local, int *nice_remote, int *ionice_remote)
{
...
/**
 * Parse the given argument and set nice and ionice accordingly
 */
int niceness_parse_argument(const char * arg)
{
	return parse_argument(arg, &niceness.nice_local, &niceness.ionice_local, &niceness.nice_remote, &niceness.ionice_remote);
}
