/*
 * Test harness for vfs_chmod_at(). Confirms the symlink-TOCTOU
 * primitive used by CVE-2026-29518 (and its incomplete-fix follow-up
 * for chmod) is closed by vfs_chmod_at(): a parent directory component
...
/* Does vfs_chmod_at()'s leaf handling refuse to follow a symlink at the final
...
int main(int argc, char **argv)
{
	...
	/* Simulate the daemon-without-chroot deployment that vfs_chmod_at()
	...
	int rc = vfs_chmod_at("inside_link/sentinel", 0640);
	...
	rc = vfs_chmod_at("escape_link/sentinel", 0666);
	 ...
	 * normal case. */
	rc = vfs_chmod_at("realdir/sentinel", 0644);
	...
	if (leaf_chmod_nofollow_supported()) {
		rc = vfs_chmod_at("realdir/leaflink", 0666);
