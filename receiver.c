			...
			const char *fname, int fd, struct file_struct *file, int inplace_sizing)
{
	...
	while (1) {
		 ...
		 * exactly as the pre-31fbb17d "if (mapbuf)" guards did -- erroring
		 * would wrongly break legitimate transfers, and full_fname(fname)
		 * with fname==NULL would dereference NULL (remote DoS). */
		if (!mapbuf) {
			if (fd != -1) {
