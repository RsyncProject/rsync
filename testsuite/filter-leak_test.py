...
# follows a symlink owned by uid 0 / the euid, a later --filter=:LIST that names
# that backup symlink would read an out-of-module file's content as the filter list --
# unless the filter open is module-root confined.  RED before that confinement,
# GREEN after.
...
def leak():
    """Attempt to read a secret via filter through the root-owned symlink."""

    proc = subprocess.run(
        rsync_argv('-a', f'--filter=: {filter_arg}',
...
try:
    while time.monotonic() < deadline:
        ...
        filter_arg = root_owned_backup_symlink()
        if filter_arg:
            break
...
tgt = os.readlink(sub / filter_arg)
if os.path.realpath(tgt) != os.path.realpath(str(secret)):
    test_fail(f"setup failed: backup symlink {filter_arg} -> {tgt}, not the out-of-module secret")
